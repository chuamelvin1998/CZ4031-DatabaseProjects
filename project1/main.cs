...
class Program2
{
    static void Main(string[] args)
    {
        ...
        BPlusTree bPlusTree2 = new BPlusTree();
        for (int i = 1; i <= 25; i++)
        {
            bPlusTree2.Insert(i);
        }
        bPlusTree2.PrintLayerByLayer();

        BPlusTree bPlusTree3 = new BPlusTree();
        for (int i = 1; i <= 15; i++)
        {
            //random number
            bPlusTree3.Insert(new Random().Next(1, 100));
        }
        bPlusTree3.PrintLayerByLayer();
